...
void modal_setup_esc_menu(Modal *modal, int screen_width, int screen_height) {
  ...
  modal->bounds = (Rectangle){(screen_width - modal_width) / 2.0f,
...
void modal_setup_level_up(Modal *modal, int screen_width, int screen_height,
  ...
  for (int i = 0; i < class_count; i++) {
    ...
    button_init(&modal->buttons[i], button_x, button_y, button_width,
                button_height);
    button_set_label(&modal->buttons[i], class_options[i], 22, BLACK);
    button_set_colors(&modal->buttons[i], (Color){200, 220, 255, 255},
                      DARKBLUE);
    ...
    if (modal->buttons[i].pressed && mouse_pressed) {
      ...
      case MODAL_TYPE_ESC_MENU:
        if (i == 0)
          ...
          modal->result = MODAL_RESULT_EXIT_TO_MENU;
        else if (i == 2)
          modal->result = MODAL_RESULT_EXIT_TO_DESKTOP;
        break;
...
/**
 * @brief Render an active modal dialog including overlay, background, text, and
 * buttons.
...
void modal_render(const Modal *modal) {
  ...
  DrawText(modal->title,
           ...
           modal->bounds.y + 20, title_size, BLACK);
  ...
  if (modal->description[0] != '\0') {
    ...
    DrawText(modal->description,
             ...
             modal->bounds.y + 60, desc_size, DARKGRAY);
  }

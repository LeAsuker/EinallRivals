...
/**
 * @brief Simple UI button primitive used for interactive elements outside the
 * map. Coordinates are in pixels (screen space).
 */
...
/**
 * @brief Initialize a button's rectangle and pressed state with sensible
 * defaults.
 ...
 */
static inline void button_set_rect(Button *b, int x, int y, int width,
                                   int height) {
  b->x = x;
  b->y = y;
  b->width = width;
 ...
 *        Pass NULL or empty string to clear the label. `text_size` is font size
 * in pixels.
 * @param b Button to modify.
 ...
 */
static inline void button_set_label(Button *b, const char *text, int text_size,
                                    Color text_color) {
  if (text == NULL) {
    b->label[0] = '\0';
  } else {
...
/**
 * @brief Update the pressed state based on whether the mouse is currently down
 * and whether the cursor is over the button. Call every frame before rendering.

...
bool button_is_mouse_over(const Button *b) {
  ...
  int mx = GetMouseX();
  int my = GetMouseY();
  return (mx >= b->x && mx < b->x + b->width && my >= b->y &&
          my < b->y + b->height);
}

/**
 * @brief Update the button pressed state based on mouse position and
 * button-down state.
...
void button_draw(const Button *b) {
  ...
  for (int i = 0; i < b->border_thickness; i++) {
    DrawRectangleLines(b->x - i, b->y - i, b->width + i * 2, b->height + i * 2,
                       b->border_color);
  }
...
// No-op color setter here since header exposes inline setter; kept for
// compatibility (defined inline in header)

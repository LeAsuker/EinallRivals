...
void menu_update(MenuState *state) {
  ...
  if (mouse_x >= start_button_x &&
      mouse_x <= start_button_x + MENU_BUTTON_WIDTH &&
      mouse_y >= start_button_y &&
      mouse_y <= start_button_y + MENU_BUTTON_HEIGHT) {
    state->hovered_option = MENU_START;
  } else if (mouse_x >= quit_button_x &&
             mouse_x <= quit_button_x + MENU_BUTTON_WIDTH &&
             mouse_y >= quit_button_y &&
             mouse_y <= quit_button_y + MENU_BUTTON_HEIGHT) {
...
void menu_render(MenuState *state, int screen_width, int screen_height) {
  ...
  DrawRectangle(start_button_x, start_button_y, MENU_BUTTON_WIDTH,
                MENU_BUTTON_HEIGHT, start_color);
  DrawRectangleLines(start_button_x, start_button_y, MENU_BUTTON_WIDTH,
                     MENU_BUTTON_HEIGHT, BLACK);
  ...
  DrawText(start_text,
           ...
           start_button_y + MENU_BUTTON_HEIGHT / 2 - 10, 20, WHITE);

  // Draw Quit button
  Color quit_color = (state->hovered_option == MENU_QUIT) ? RED : DARKPURPLE;
  DrawRectangle(quit_button_x, quit_button_y, MENU_BUTTON_WIDTH,
                MENU_BUTTON_HEIGHT, quit_color);
  DrawRectangleLines(quit_button_x, quit_button_y, MENU_BUTTON_WIDTH,
                     MENU_BUTTON_HEIGHT, BLACK);
  ...
  DrawText(quit_text,
           ...
           quit_button_y + MENU_BUTTON_HEIGHT / 2 - 10, 20, WHITE);

...
CombatResult combat_execute(Character *attacker, Character *defender) {
  ...
  // New rule: battle skills are single-sided actions used during the actor's
  // turn. They hit deterministically and do raw physical attack equal to
  // attacker's phys_attack.
...
CombatForecast combat_forecast(Character *attacker, Character *defender) {
  ...
  forecast.defender_health_after =
      defender->curr_health - forecast.attacker_damage;
  if (forecast.defender_health_after < 0)
    forecast.defender_health_after = 0;
...
/**
 * @brief Determine whether an attacker cell can perform an attack on a defender
 * cell.
 *
...
/**
 * @brief Compute damage for an interaction, delegating to physical or magical
 * formulas.
 *
 * @param attacker Attacking character.
 * @param defender Defending character.
 ...
 */
int combat_calculate_damage(Character *attacker, Character *defender,
                            bool is_magic) {
  if (is_magic) {
...
/**
 * @brief Calculate physical damage as attacker's phys_attack minus defender
 * phys_defense. Ensures a minimum of 1 damage.
 */
...
/**
 * @brief Calculate magical damage as attacker's magic_attack minus defender
 * magic_defense. Ensures a minimum of 1 damage.
 */
 ...
 */
int combat_calculate_experience(Character *attacker, Character *defender,
                                bool killed) {
 ...
 */
void combat_grant_experience(Character *attacker, Character *defender,
                             bool killed) {
...
/**
 * @brief Apply damage to a character using the public character_take_damage
 * helper.
 */
static void apply_combat_damage(Character *character, int damage) {

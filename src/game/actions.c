 ...
 * @return Populated ActionIcons struct.
 */
ActionIcons action_icons_load(void) {
  ...
  RAYLIB_CHECK_TEXTURE(icons.spear_strike);
  // If there were other action icons, load them similarly, e.g.:
  // icons.bite = LoadTexture("actions/bite_icon.png");
  // RAYLIB_CHECK_TEXTURE(icons.bite);
  return icons;
}
